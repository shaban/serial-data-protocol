// Criterion benchmark for arrays using canonical fixtures
// Uses statistical analysis to handle variance and detect outliers
...
fn load_test_data() -> (ArraysOfPrimitives, Vec<u8>) {
    ...
    // Criterion runs from project root
...
fn bench_encode(c: &mut Criterion) {
    ...
    c.bench_function("arrays_encode", |b| {
        b.iter(|| {
...
fn bench_decode(c: &mut Criterion) {
    ...
    c.bench_function("arrays_decode", |b| {
        b.iter(|| {
...
fn bench_roundtrip(c: &mut Criterion) {
    ...
    c.bench_function("arrays_roundtrip", |b| {
        b.iter(|| {
            let size = data.encode_to_slice(&mut buf).unwrap();
