...
// generateBulkArrayDecode generates optimized bulk copy code for primitive integer/float arrays
// Uses experimental fast-path: aligned cast, memcpy-aligned-cast, or ptr::read_unaligned
func generateBulkArrayDecode(buf *strings.Builder, elemType, fieldName, indent string) {
	elemSize := FixedSize(elemType)

	buf.WriteString(fmt.Sprintf("%s// Experimental fast-path decode for primitive arrays\n", indent))
	buf.WriteString(fmt.Sprintf("%slet %s = if array_len > 0 {\n", indent, fieldName))

	if elemType == "u8" || elemType == "i8" {
		// Single-byte types: direct slice copy (no alignment issues)
	...
	} else if elemType == "u32" {
		// Use experimental fast-path for u32
		buf.WriteString(fmt.Sprintf("%s    let byte_len = array_len * 4;\n", indent))
		...
		buf.WriteString(fmt.Sprintf("%s    wire_slice::decode_u32_array_fast(bytes, array_len)?\n", indent))
	} else if elemType == "f64" {
		// Use experimental fast-path for f64
		buf.WriteString(fmt.Sprintf("%s    let byte_len = array_len * 8;\n", indent))
		buf.WriteString(fmt.Sprintf("%s    wire_slice::check_bounds(buf, offset, byte_len)?;\n", indent))
		buf.WriteString(fmt.Sprintf("%s    let bytes = &buf[offset..offset + byte_len];\n", indent))
		buf.WriteString(fmt.Sprintf("%s    offset += byte_len;\n", indent))
		buf.WriteString(fmt.Sprintf("%s    wire_slice::decode_f64_array_fast(bytes, array_len)?\n", indent))
	} else {
		// Other multi-byte types: use try_cast_slice with chunks_exact fallback
