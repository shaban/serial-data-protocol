...
func generateStructDecode(buf *strings.Builder, s parser.Struct) error {
	...
	buf.WriteString("    /// Decode from byte array\n")
...
func decodePrimitive(buf *strings.Builder, typeName, fieldName, indent string) error {
	switch typeName {
	case "u8":
		buf.WriteString(fmt.Sprintf("%sguard offset < bytes.count else { throw SDPDecodeError.insufficientData }\n", indent))
		buf.WriteString(fmt.Sprintf("%slet %s = bytes[offset]\n", indent, fieldName))
		buf.WriteString(fmt.Sprintf("%soffset += 1\n", indent))

	case "i8":
		buf.WriteString(fmt.Sprintf("%sguard offset < bytes.count else { throw SDPDecodeError.insufficientData }\n", indent))
		buf.WriteString(fmt.Sprintf("%slet %s = Int8(bitPattern: bytes[offset])\n", indent, fieldName))
		buf.WriteString(fmt.Sprintf("%soffset += 1\n", indent))

	case "u16":
		buf.WriteString(fmt.Sprintf("%sguard offset + 2 <= bytes.count else { throw SDPDecodeError.insufficientData }\n", indent))
		buf.WriteString(fmt.Sprintf("%slet %s = bytes[offset..<offset+2].withUnsafeBytes { UInt16(littleEndian: $0.load(as: UInt16.self)) }\n", indent, fieldName))
		buf.WriteString(fmt.Sprintf("%soffset += 2\n", indent))

	case "u32":
		buf.WriteString(fmt.Sprintf("%sguard offset + 4 <= bytes.count else { throw SDPDecodeError.insufficientData }\n", indent))
		buf.WriteString(fmt.Sprintf("%slet %s = bytes[offset..<offset+4].withUnsafeBytes { UInt32(littleEndian: $0.load(as: UInt32.self)) }\n", indent, fieldName))
		buf.WriteString(fmt.Sprintf("%soffset += 4\n", indent))

	case "u64":
		buf.WriteString(fmt.Sprintf("%sguard offset + 8 <= bytes.count else { throw SDPDecodeError.insufficientData }\n", indent))
		buf.WriteString(fmt.Sprintf("%slet %s = bytes[offset..<offset+8].withUnsafeBytes { UInt64(littleEndian: $0.load(as: UInt64.self)) }\n", indent, fieldName))
		buf.WriteString(fmt.Sprintf("%soffset += 8\n", indent))

	case "i16":
		buf.WriteString(fmt.Sprintf("%sguard offset + 2 <= bytes.count else { throw SDPDecodeError.insufficientData }\n", indent))
		buf.WriteString(fmt.Sprintf("%slet %s = bytes[offset..<offset+2].withUnsafeBytes { Int16(bitPattern: UInt16(littleEndian: $0.load(as: UInt16.self))) }\n", indent, fieldName))
		buf.WriteString(fmt.Sprintf("%soffset += 2\n", indent))

	case "i32":
		buf.WriteString(fmt.Sprintf("%sguard offset + 4 <= bytes.count else { throw SDPDecodeError.insufficientData }\n", indent))
		buf.WriteString(fmt.Sprintf("%slet %s = bytes[offset..<offset+4].withUnsafeBytes { Int32(bitPattern: UInt32(littleEndian: $0.load(as: UInt32.self))) }\n", indent, fieldName))
		buf.WriteString(fmt.Sprintf("%soffset += 4\n", indent))

	case "i64":
		buf.WriteString(fmt.Sprintf("%sguard offset + 8 <= bytes.count else { throw SDPDecodeError.insufficientData }\n", indent))
		buf.WriteString(fmt.Sprintf("%slet %s = bytes[offset..<offset+8].withUnsafeBytes { Int64(bitPattern: UInt64(littleEndian: $0.load(as: UInt64.self))) }\n", indent, fieldName))
		buf.WriteString(fmt.Sprintf("%soffset += 8\n", indent))

	case "f32":
		buf.WriteString(fmt.Sprintf("%sguard offset + 4 <= bytes.count else { throw SDPDecodeError.insufficientData }\n", indent))
		buf.WriteString(fmt.Sprintf("%slet %sBits = bytes[offset..<offset+4].withUnsafeBytes { UInt32(littleEndian: $0.load(as: UInt32.self)) }\n", indent, fieldName))
		buf.WriteString(fmt.Sprintf("%slet %s = Float(bitPattern: %sBits)\n", indent, fieldName, fieldName))
		buf.WriteString(fmt.Sprintf("%soffset += 4\n", indent))

	case "f64":
		buf.WriteString(fmt.Sprintf("%sguard offset + 8 <= bytes.count else { throw SDPDecodeError.insufficientData }\n", indent))
		buf.WriteString(fmt.Sprintf("%slet %sBits = bytes[offset..<offset+8].withUnsafeBytes { UInt64(littleEndian: $0.load(as: UInt64.self)) }\n", indent, fieldName))
		buf.WriteString(fmt.Sprintf("%slet %s = Double(bitPattern: %sBits)\n", indent, fieldName, fieldName))
		buf.WriteString(fmt.Sprintf("%soffset += 8\n", indent))

	case "bool":
		buf.WriteString(fmt.Sprintf("%sguard offset < bytes.count else { throw SDPDecodeError.insufficientData }\n", indent))
		buf.WriteString(fmt.Sprintf("%slet %sByte = bytes[offset]\n", indent, fieldName))
	...
	case "str":
		...
		buf.WriteString(fmt.Sprintf("%sguard offset + 4 <= bytes.count else { throw SDPDecodeError.insufficientData }\n", indent))
		buf.WriteString(fmt.Sprintf("%slet %sLen = Int(bytes[offset..<offset+4].withUnsafeBytes { UInt32(littleEndian: $0.load(as: UInt32.self)) })\n", indent, fieldName))
		buf.WriteString(fmt.Sprintf("%soffset += 4\n", indent))
		buf.WriteString(fmt.Sprintf("%sguard offset + %sLen <= bytes.count else { throw SDPDecodeError.insufficientData }\n",
			indent, fieldName))
		buf.WriteString(fmt.Sprintf("%slet %sBytes = Array(bytes[offset..<offset+%sLen])\n", indent, fieldName, fieldName))
		buf.WriteString(fmt.Sprintf("%sguard let %s = String(bytes: %sBytes, encoding: .utf8) else {\n",
			indent, fieldName, fieldName))
...
func generateOptionalDecode(buf *strings.Builder, field parser.Field, indent string) error {
	...
	buf.WriteString(fmt.Sprintf("%slet %sPresent = bytes[offset]\n", indent, fieldName))
	buf.WriteString(fmt.Sprintf("%soffset += 1\n", indent))
