...
std::vector<uint8_t> EncodePointMessage(const Point& src) {
    // Calculate payload size
    size_t payloadSize = point_size(src);
    ...
    *(uint32_t*)(buf + 6) = SDP_HTOLE32(static_cast<uint32_t>(payloadSize));  // Payload length
    ...
    // Encode payload
    point_encode(src, buf + MESSAGE_HEADER_SIZE);

    return message;
}

std::vector<uint8_t> EncodeRectangleMessage(const Rectangle& src) {
    // Calculate payload size
    size_t payloadSize = rectangle_size(src);
    ...
    *(uint32_t*)(buf + 6) = SDP_HTOLE32(static_cast<uint32_t>(payloadSize));  // Payload length
    ...
    // Encode payload
    rectangle_encode(src, buf + MESSAGE_HEADER_SIZE);

    return message;
}

std::vector<uint8_t> EncodeSceneMessage(const Scene& src) {
    // Calculate payload size
    size_t payloadSize = scene_size(src);
    ...
    *(uint32_t*)(buf + 6) = SDP_HTOLE32(static_cast<uint32_t>(payloadSize));  // Payload length
    ...
    // Encode payload
    scene_encode(src, buf + MESSAGE_HEADER_SIZE);
