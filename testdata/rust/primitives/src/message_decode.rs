...
/// Decodes a AllPrimitives from self-describing message format.
///
...
pub fn decode_all_primitives_message(data: &[u8]) -> Result<AllPrimitives, MessageError> {
    ...
    AllPrimitives::decode_from_slice(payload)
...
///     Message::AllPrimitives(val) => { /* handle AllPrimitives */ },
/// }
/// ```
pub fn decode_message(data: &[u8]) -> Result<Message, MessageError> {
    ...
    match type_id {
        1 => {
            decode_all_primitives_message(data).map(Message::AllPrimitives)
        }
