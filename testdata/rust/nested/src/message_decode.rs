...
/// Decodes a Point from self-describing message format.
///
...
/// Decodes a Rectangle from self-describing message format.
///
/// Validates the 10-byte header:
/// - Magic bytes must be 'SDP'
/// - Version must be '2'
...
pub fn decode_rectangle_message(data: &[u8]) -> Result<Rectangle, MessageError> {
    ...
    // Decode payload using byte mode decoder
    Rectangle::decode_from_slice(payload)
        .map_err(|e| MessageError::DecodeError(e.to_string()))
}

/// Decodes a Scene from self-describing message format.
///
/// Validates the 10-byte header:
/// - Magic bytes must be 'SDP'
/// - Version must be '2'
...
///
/// Returns an error if validation fails or payload is invalid.
pub fn decode_scene_message(data: &[u8]) -> Result<Scene, MessageError> {
    // Check minimum message size
    if data.len() < MESSAGE_HEADER_SIZE {
        return Err(MessageError::MessageTooShort);
    }
    ...
    // Decode payload using byte mode decoder
    Scene::decode_from_slice(payload)
        .map_err(|e| MessageError::DecodeError(e.to_string()))
}
...
///     Message::Point(val) => { /* handle Point */ },
///     // ... other variants
/// }
/// ```
pub fn decode_message(data: &[u8]) -> Result<Message, MessageError> {
    ...
    match type_id {
        1 => {
            decode_point_message(data).map(Message::Point)
        }
        2 => {
            decode_rectangle_message(data).map(Message::Rectangle)
        }
        3 => {
            decode_scene_message(data).map(Message::Scene)
        }
