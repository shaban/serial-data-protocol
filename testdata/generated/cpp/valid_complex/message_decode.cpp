...
DeviceList DecodeDeviceListMessage(const std::vector<uint8_t>& data) {
    ...
    if (typeID != 1) {
        throw MessageDecodeError("Wrong type ID: expected 1 (DeviceList), got " + std::to_string(typeID));
    }
    ...
    // Decode payload
    return device_list_decode(data.data() + MESSAGE_HEADER_SIZE, payloadLength);
}
...
Device DecodeDeviceMessage(const std::vector<uint8_t>& data) {
    ...
    uint16_t typeID = SDP_LE16TOH(*(const uint16_t*)(data.data() + 4));
    if (typeID != 2) {
        throw MessageDecodeError("Wrong type ID: expected 2 (Device), got " + std::to_string(typeID));
    }
    ...
    // Decode payload
    return device_decode(data.data() + MESSAGE_HEADER_SIZE, payloadLength);
}
...
Parameter DecodeParameterMessage(const std::vector<uint8_t>& data) {
    ...
    uint16_t typeID = SDP_LE16TOH(*(const uint16_t*)(data.data() + 4));
    if (typeID != 3) {
        throw MessageDecodeError("Wrong type ID: expected 3 (Parameter), got " + std::to_string(typeID));
    }
