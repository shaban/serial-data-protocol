...
std::vector<uint8_t> EncodeArraysOfPrimitivesMessage(const ArraysOfPrimitives& src) {
    ...
    size_t payloadSize = arrays_of_primitives_size(src);
    ...
    *(uint32_t*)(buf + 6) = SDP_HTOLE32(static_cast<uint32_t>(payloadSize));  // Payload length
    ...
    // Encode payload
    arrays_of_primitives_encode(src, buf + MESSAGE_HEADER_SIZE);
...
std::vector<uint8_t> EncodeItemMessage(const Item& src) {
    // Calculate payload size
    size_t payloadSize = item_size(src);
    ...
    *(uint32_t*)(buf + 6) = SDP_HTOLE32(static_cast<uint32_t>(payloadSize));  // Payload length
    ...
    // Encode payload
    item_encode(src, buf + MESSAGE_HEADER_SIZE);
...
std::vector<uint8_t> EncodeArraysOfStructsMessage(const ArraysOfStructs& src) {
    // Calculate payload size
    size_t payloadSize = arrays_of_structs_size(src);
    ...
    *(uint32_t*)(buf + 6) = SDP_HTOLE32(static_cast<uint32_t>(payloadSize));  // Payload length
    ...
    // Encode payload
    arrays_of_structs_encode(src, buf + MESSAGE_HEADER_SIZE);
