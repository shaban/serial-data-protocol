...
Example DecodeExampleMessage(const std::vector<uint8_t>& data) {
    ...
    if (typeID != 1) {
        throw MessageDecodeError("Wrong type ID: expected 1 (Example), got " + std::to_string(typeID));
    }
    ...
    // Decode payload
    return example_decode(data.data() + MESSAGE_HEADER_SIZE, payloadLength);
}
