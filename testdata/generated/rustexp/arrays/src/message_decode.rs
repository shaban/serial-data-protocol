...
pub enum Message {
    ...
    Item(Item),
    ArraysOfStructs(ArraysOfStructs),
}

/// Decodes a ArraysOfPrimitives from self-describing message format.
///
...
pub fn decode_arrays_of_primitives_message(data: &[u8]) -> Result<ArraysOfPrimitives, MessageError> {
    ...
    ArraysOfPrimitives::decode_from_slice(payload)
        .map_err(|e| MessageError::DecodeError(e.to_string()))
}

/// Decodes a Item from self-describing message format.
///
/// Validates the 10-byte header:
/// - Magic bytes must be 'SDP'
/// - Version must be '2'
...
///
/// Returns an error if validation fails or payload is invalid.
pub fn decode_item_message(data: &[u8]) -> Result<Item, MessageError> {
    // Check minimum message size
    if data.len() < MESSAGE_HEADER_SIZE {
        return Err(MessageError::MessageTooShort);
    }
    ...
    // Decode payload using byte mode decoder
    Item::decode_from_slice(payload)
        .map_err(|e| MessageError::DecodeError(e.to_string()))
}

/// Decodes a ArraysOfStructs from self-describing message format.
///
/// Validates the 10-byte header:
/// - Magic bytes must be 'SDP'
/// - Version must be '2'
...
///
/// Returns an error if validation fails or payload is invalid.
pub fn decode_arrays_of_structs_message(data: &[u8]) -> Result<ArraysOfStructs, MessageError> {
    // Check minimum message size
    if data.len() < MESSAGE_HEADER_SIZE {
        return Err(MessageError::MessageTooShort);
    }
    ...
    // Decode payload using byte mode decoder
    ArraysOfStructs::decode_from_slice(payload)
        .map_err(|e| MessageError::DecodeError(e.to_string()))
}
...
///     Message::ArraysOfPrimitives(val) => { /* handle ArraysOfPrimitives */ },
///     // ... other variants
/// }
/// ```
pub fn decode_message(data: &[u8]) -> Result<Message, MessageError> {
    ...
    match type_id {
        1 => {
            decode_arrays_of_primitives_message(data).map(Message::ArraysOfPrimitives)
        }
        2 => {
            decode_item_message(data).map(Message::Item)
        }
        3 => {
            decode_arrays_of_structs_message(data).map(Message::ArraysOfStructs)
        }
